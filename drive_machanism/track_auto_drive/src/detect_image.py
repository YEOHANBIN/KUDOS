...
roi_mode_T = (0,0)
roi_mode_B = (0,0)
roi_track_T = (int(1280*0.4),int(0))
roi_track_B = (int(1280*0.5),int(360*0.2))
running = True
...
def color_callback(data):
    global average_color, pixel_count, running, roi_mode_T,roi_mode_B, roi_track_T, roi_track_B
    ...
    roi_track = color_image[roi_track_T[1]:roi_track_B[1], roi_track_T[0]:roi_track_B[0]]


    # 특정 영역을 추출
    #roi = color_image[roi_top_left[1]:roi_bottom_right[1], roi_top_left[0]:roi_bottom_right[0]]
    ...
    average_color_track = np.mean(roi_track, axis=(0, 1))

    # 출력
    print("Pixel Count_1:", pixel_count_1)
    print("Average Color_HSV:", average_color_hsv)
    print('\n')
    print("Pixel Count_1:", pixel_count_2)
    print("Average Color_BGR:", average_color_bgr)

    print("Average Track Color:", average_color_track)
    print('====================================================')
    ...
    cv2.rectangle(color_image,
                  (int(roi_track_T[0]), int(roi_track_T[1])),
                  (int(roi_track_B[0]), int(roi_track_B[1])),
                  (255, 0, 0), 2)
...
def listener():
    ...
    pub = rospy.Publisher('image_info', Int8, queue_size=10)
