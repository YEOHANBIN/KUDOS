...
def SLOPE_L_WINTER():
    ...
    slope_flag += 1
    if slope_flag < 3:
        message = 30003000-serial_directory["Go"]
    elif slope_flag < 5:
...
def modeCB(data):
    ...
    if image_stack[0] > 10 or image_stack[1] > 10:
        ...
        if mode == 0: message = serial_directory["BACK"]
        if mode == 1: SLOPE_R()
        if mode == 2: SLOPE_L_WINTER()
        if mode == 3: message = serial_directory["BACK"]-20
...
def winter_mode():
    ...
    rospy.init_node('winter', anonymous=True)
